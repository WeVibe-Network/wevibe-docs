...
# 26 — Known Live Defects Register

*Live silent-failure paths across the memory pipeline, highest severity first. Each entry: **What** (the failure) and **Why** (how it arose). Anchors verified on-touch 2026-08-21. Net-new index — it does not relocate the existing inline F1 (LIVE BUG, §24.C:2459) / F2 (reconcile, §14:1640) flags, which stay in their sections.*

## C1 — `memory_keywords` INSERT fails at runtime (schema dropped `weight`, watcher not updated) — HIGHEST SEVERITY · live silent failure

**What.** `memory_keywords` has no `weight` column (`db/schema.sql:599-605` — columns `memory_cid`, `org_id`, `keyword` only), but the chain watcher still INSERTs one (`wevibe-server/wevibe-hub/internal/chain/watcher_memory.go:161-165`: `INSERT INTO memory_keywords (memory_cid, org_id, keyword, weight) … ON CONFLICT … DO UPDATE SET weight = $4`). PostgreSQL deterministically raises `column "weight" of relation "memory_keywords" does not exist`, and the error is swallowed into a `logger.Warn` (`watcher_memory.go:166-168`); `pending_submissions` still flips to `committed` (`:176-183`). The `org_keywords` INSERT matches its schema (`watcher_memory.go:152-156`) and succeeds, so the SQL keyword-join half of recall has carried nothing since the pivot, with no error anywhere.

**Why.** The recall pivot (commit `9ccee0d`, 2026-07-29) deliberately dropped `memory_keywords.weight` (flat keyword labels; weights moved to the Qdrant payload, `watcher_memory.go:94-114`, see RECALL-PIVOT-SPEC.md:249) but the watcher was not updated to match. The e2e suite never asserts `memory_keywords` rows (asserts only status=`committed` + Qdrant recall), so the failure is invisible. Rows predating `9ccee0d` persist (EXTRACTION-FLOW.md:153 records 185 for `wevibe-org-0`), so "never populates" is true only since the pivot. Corollary: TOPOLOGY.md:607's "ORPHANED — 0 source refs" for `memory_keywords` is itself stale (live reader `keywords.go:64,286,380`).

## C2 — Provenance stamp half-unwired: `attestation_session_hash` never set; `producer_model_id` bench-only

**What.** The on-chain commitment `StoredMemoryCommitment` carries 22 fields (`wevibe-chain/x/memory/types/state.pb.go:255-282`), including `producer_model_id` (field 24, `state.proto:74`) and `attestation_session_hash` (field 25, `state.proto:76`). `attestation_session_hash` is set by **no writer anywhere** (MsgSubmitCommitment field 17 is never encoded in any repo). `producer_model_id` is encoded only by the bench leader-signer (`wevibe-bench/scaffold/leader-signer/src/cli.ts:207,215,248`; `vendor/chain-client.ts:175-179`) — the production dashboard chain client never encodes field 16 (`dashboard/lib/chain-client.ts:141-178`), even though the review path writes `producerModelId` into the hub DB (`memory-review.tsx:255-259` → `moderation.go:128-140`). The chain's rule "producer_model_id is required when attestation_session_hash is set" (`x/memory/types/msgs.go:31-38`) therefore never fires — the hash is never set by any client.

**Why.** The stamp was designed into message + keeper (the keeper round-trips both fields, `msg_server.go:79-80` → `pendingToStored` → store) but only the bench scaffold was wired to fill `producer_model_id`, and nothing was wired to fill `attestation_session_hash`. Net: on-chain provenance exists only on the bench; in production the model id never reaches the chain and the session hash never exists. Same family: both hand-rolled chain clients encode keywords at tag `0x22` (field 4, now reserved) instead of the current flat `repeated string keywords = 8`, so keyword labels reach the chain empty from both broadcast paths.

## C3 — `language` Qdrant payload index registered but never written → zero-match filter

**What.** `language` is registered as a Qdrant keyword payload index for every org collection (`retrieval.go:338`, `ensureCollectionPayloadIndexes`). No code path ever writes it into the payload: the initial payload map has no `language` key (`retrieval.go:405-435`), `protocol.IndexEntry` has no `Language` field (`types.go:347-365`), and the update-only sites (`UpdateStanding :1312`, `setPointKeywords :1484`, `UpdateMemoryState :1540`) don't touch it. The MCP drops the plugin's `language` before the hub query — it survives only into a built-then-discarded `needCardText` (`retrieve-cli.ts:310-312`), and the hub query passes no language (`retrieve-cli.ts:387-398`; `QueryRequest` has no `Language` field, `types.go:253-267`). A filter on `language` matches zero memories: no error, no match.

**Why.** `language` entered `retrieval.go` only as the index registration itself (commit `cbaf289`, 2026-07-08); no commit ever wrote it into a payload. The plugin does collect the signal at the edge (`wevibe-plugin.ts:1402`), so the data exists and dies in a logged-but-unused string — the index was created before any writer or filter.

## C4 — Two fail-quiet paths: `sanitize.ScanContent` never called in production; ban-leg UPDATE binds the wrong value

**What.** (a) `sanitize.ScanContent` (`internal/sanitize/scanner.go:22`) has 13 callers, all tests — zero production callers; the `sanitize` package is imported nowhere in the hub. Both submit handlers pass `nil` findings (`handlers/moderation.go:109` and `:240`), so `sanitization_findings` (`schema.sql:159`) is always NULL via the only INSERT (`moderation.go:131,138`). (b) The ban-leg UPDATE (`chain/watcher_report_org.go:36-44`) binds `txHash` into the `submission_hash` WHERE clause (`$2 = txHash`, `:44`), but `submission_hash` holds lowercase hex of `sha256(ciphertext || wrapped_dek)` (`moderation.go:70-75`) while `txHash` is the uppercase Cosmos tx hash (`watcher.go:312`). Different preimage + casing → the WHERE never matches, 0 rows updated, and `RowsAffected` is never checked — no error, no effect.

**Why.** (a) `ScanContent` needs plaintext, but CO-029 removed the `Plaintext` field from `SubmitMemoryRequest` (`types.go:126-148`); the hub no longer receives plaintext by design, so the scanner is orphaned (exists-but-unreachable). Downstream readers (`memory-review.tsx:292-293`, `moderator-review-panel.tsx:322-324`) read the always-NULL field, so the badge never renders. (b) The HTTP counterpart does the identical UPDATE with the correct value (`handlers/reports.go:667-672` binds `memoryCID`), and the same function's first UPDATE binds `tx_hash = $1` correctly (`watcher_report_org.go:17`) — the ban leg alone used the wrong argument. Consequence: Qdrant point deletion still fires (`:28-34`), masking the defect, but the DB-level `banned` flag (read at `retrieval.go:400`) is never set, so a re-index would re-expose the memory.

## D1 — `org_trajectories` vocabulary table (exists, uncommitted; chain-cold at the memory hop)

**What.** `org_trajectories` already exists in the working tree (uncommitted, branch `feat/session-sources-and-provider-authority`): table `db/schema.sql:571-580` (`id`, `org_id` FK, `trajectory`, `created_at`, `deprecated`, `UNIQUE(org_id,trajectory)`); handlers `internal/api/handlers/trajectories.go`; routes `GET /v1/orgs/{orgID}/trajectories` and `POST /v1/orgs/{orgID}/trajectories` (`main.go:356-357`). Reads are member-level (`trajectories.go:59-62`), writes leader-only (`trajectories.go:121-125`). Schema mirrors `org_keywords` 1:1, but the route surface is a 2-route subset (`org_keywords` has 6) and there is no `memory_trajectories` join table or watcher upsert — committed memories get keywords but never trajectory labels; nothing classifies or matches against it yet.

**Why.** Trajectories is vocabulary management wired end-to-end (table → routes → handlers → dashboard `org-settings` UI) but uncommitted, and cold at the "trajectory label on a memory" hop: the chain watcher's vocab-join-at-commit upserts `org_keywords`/`memory_keywords` (`watcher_memory.go:150-169`) with no trajectory equivalent. Recorded as existing-but-incomplete, not proposed.

---

*Sources (WO-35, verified on-touch 2026-08-21): `wevibe-meta/workspace/reports/1787312182-WO-35-TOPOLOGY-defects-trajectories.md` — all C1–C4/D1 anchors and file:line cites confirmed against live code in that report's GATHER and IMPLEMENT sections.*
