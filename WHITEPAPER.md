...
**Four-button approval UX (D-RECALL-FEEDBACK-FOUR-BUTTON).** Plugin offers: [Accept + Attest] (memory injected, serve attestation queued, contributor earns), [Deny] (memory blocked for this session/context only — a neutral context signal, NOT a corpus down-vote; no denial event emitted to the chain), [Block] (permanent personal blacklist AND a global corpus denial signal — the load-bearing negative-signal path that feeds Earned-Trust decay via `MsgSubmitDenialBatch`), [Report] (memory blocked and escalated into the org's moderation/accountability path — see Sections 5.5–5.8). The Deny-vs-Block split is load-bearing: Deny is a no-op on the corpus (context ≠ quality); Block is the negative signal that drives decay. Public orgs with payouts may require attestation.
