...
- The main wallet's funds are never at risk (grant is scoped to Echo messages only)
...
**Why:** Direct dependency on a GPL library would force the entire Echo codebase to GPL-3.0. The sidecar pattern isolates the GPL boundary to a single process, preserving Apache-2.0 licensing for everything else. The sidecar exposes:
...
**Note on bootstrap grace duration:** The 14-epoch bootstrap grace period is measured in **chain epochs** (D-4.7), not wall time. The actual wall-clock duration depends on the chain's epoch configuration at initialization (query via `wevibed query epochs epoch-info echo_epoch`). With a 12-hour chain epoch, 14 epochs ≈ 7 days. With a 24-hour chain epoch, 14 epochs ≈ 2 weeks. The D-4.2 comment "~2 weeks at 12h/epoch" assumes 12h epochs — verify against actual chain configuration.
...
**Decision:** "Epoch" has two distinct meanings in the Echo system; the documentation disambiguates them as **rotation epoch** and **chain epoch**.
...
- **Chain epoch:** Time-based window for emissions, idle decay, and bootstrap grace. Fixed duration (configured at chain initialization in `x/epochs` module; queryable via `wevibed query epochs epoch-info echo_epoch`).
...
- Recovery code path NOT in the hub binary (separate `echo-recover` CLI tool)
...
**Decision:** Vocabulary starter packs ("web-backend", "mobile", "devops" templates) are not built into the Echo platform. New orgs start with empty vocabulary; leaders add keywords manually via the dashboard.

**Why:** Vocabulary curation is an org responsibility, not a platform responsibility. The Echo-sponsored org will create starter packs as community contributions when the chain is live. The dashboard's `/keywords` page already provides full CRUD for keyword management — no platform-level feature is missing.
...
**Why accepted:** This is a fundamental tension in collaborative knowledge systems — it exists in Google Drive, Confluence, SharePoint, and every other shared knowledge platform. It is not unique to Echo. Mitigations (anomaly detection on serve patterns, additional controls on high-weight memories) can be added but cannot eliminate the underlying tradeoff.
...
**Decision:** Consumer risk appetite (`lowest | neutral`) is enforced client-side in both the plugin AND wevibe-mcp (via separate read implementations of the same `~/.echo/plugin-config.json`). The hub does NOT filter by risk appetite — it returns all relevant memories. The filter is applied after retrieval, before the approval UI.
...
**Accepted tradeoff:** Both the plugin (`Echo/.opencode/plugins/wevibe-guard.ts`) and wevibe-mcp (`Echo/wevibe-mcp/src/risk-appetite.ts`) implement their own `getRiskAppetite()` function reading `~/.echo/plugin-config.json`. This is the intended design, not a bug to fix by extracting shared code.
...
- The hub is open-source. Anyone can run their own hub. For now, echo-infra runs the shared hub instance that hosts multiple orgs.
...
**Task A Note:** Current Qdrant state is (B) filtered isolation — single shared collection `echo_memories` with org_id filter on every query. Implementation requires migrating to per-org collections named `org_{orgID}_memories`.
...
- Public by default aligns with the open knowledge graph goal — hiding profiles would undermine the discovery surface that makes Echo valuable.
...
**Why:**
- All-orgs aggregation is critical. Users who belong to multiple orgs need a unified view of activity across their entire Echo presence, not N separate dashboards.
...
- Event categories are broad enough to capture everything meaningful without explosion. Each category maps to a specific Echo action and produces a specific notification type.
...
| Operational queue | wevibe-hub | Pending submissions, votes, batches, chain_commit_events | Hub operator (Echo-hosted OR self-hosted) |
...
The Social Graph Service is a separate scoped CO (not part of CO-245). Both Echo-hosted hubs and self-hosted hubs consume it for human-readable names. Mandatory display name registration before a user can be accepted into moderator role (enforced at the hub-side "accept moderator role" handler — also part of the future Social Graph Service CO).
...
**Decision:** Remove `DORMANT`, `DEGRADED`, and `STABLE` from the `MemoryState` enum in `proto/echo/memory/v1/state.proto`. The locked enum values are:
...
**Why:**
- **CO-244 found rep tiers plumbed but never functionally wired.** The tier configs were stored, the lookup function existed, but it always received `0` — meaning every contributor got the lowest tier. The meritocratic emission model that justifies Echo's economic design was functionally inactive.
...
**Decision:** Before Echo ships a public testnet (and before any external operator runs an open-source hub against persisted data), a proper schema migration system MUST be implemented. The pre-MVP "wipe + recreate" model is unacceptable post-launch because:
...
**Tracked as:** GAP-T2 in MASTER.md gap log. Severity: BLOCKING-FOR-PUBLIC-LAUNCH. Must be addressed before any external user creates persistent state in an Echo hub.
...
**Decision:** The hub broadcasts chain transactions via Comet RPC `broadcast_tx_sync` (against the validator's RPC endpoint at `tcp://wevibed:26657`), not via Cosmos SDK gRPC `BroadcastTx`. Fees are calculated as `ceil(gas_limit × min_gas_price)` with `min_gas_price = 0.01 uecho` and a floor of 2000 uecho. Transient state-load errors from the local chain runtime (e.g., `version does not exist (latest height: N)` during block turnover) are retried with up to 8 attempts and exponential backoff (400ms × attempt number).
...
- **Gas-proportional fees are required for multi-message batch TXs.** D-6.6 caps batches at 500 memories per multi-message TX. Static fees that worked for single-message TXs (2000 uecho) became insufficient at multi-message gas levels (6000+ uecho), causing `insufficient fees` errors. The `gas × min_gas_price` formula scales correctly with batch size.
