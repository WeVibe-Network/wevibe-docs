...
**What was decided (the inversion).** The chain stores **EVENTS** — serve / block / outcome (use-leg) / contest / sponsorship / validity-predicate outcome / cost-to-discover attestation / convergence — immutable, append-only, content-free, consumer-signed, submitted through the hub relay — the sole built path: the chain rejects any other transaction signer (x/serve `requireServingKeySigner`), so the relay is the only path, not a fallback. **Standing** — "should this memory surface, and where" — is **COMPUTED at the edge** as a deterministic pure function of (the event stream, a published policy version whose hash is anchored on-chain). Nothing on-chain is ever a verdict: no weights, no standing, no scores, no content. The per-keyword decay formula, per-keyword weights, and the `matched_keywords` serve-gate leave consensus entirely. The load-bearing signal is **outcome** ("did it work", episode-level), not attention; serve-semantics (license vs renewable) is demoted to a revisable edge-policy draft.
...
All events share one envelope: **`(event_type, org_id, memory_cid, epoch/window, signer_pubkey, nonce, [event fields], signature)`** — ed25519 over a canonical body by the consumer's per-org serve key (D-SERVE-CONSUMER-SIGNED lineage). Every event is **content-free** (references and fingerprints only — never plaintext, keys, or memory content; D-MISSION-INVARIANT / R-37) and **consumer-signed** — the consumer signs the event body with a passkey-derived per-org key. **Two signatures, two parties:** the org's registered hub serving key signs the transaction envelope (gas carrier via org feegrant, D-S32-ORG-FEEGRANT-ALL) and cannot mint event content. The chain rejects any other transaction signer (`requireServingKeySigner`, x/serve), so the **hub relay is the only submission path — not a fallback**; consumer-direct broadcast of evidence events is not built and would be rejected. (Contrast D-REPORT-DIRECT-BROADCAST: user-signed *accountability* txs — report/expose/migrate/claim — broadcast direct to chain RPC with relay fallback; a different transaction class.)
...
| E3 | **outcome (use-leg)** | `episode_ref` (derived from the STABLE `failureKey` — **episode identity**, not session+need) · resolution tri-state `worked \| didnt_work \| unobserved` (WO-ATTRIB 2026-08-07) · `evidence_ref` (GSTV-sealed pattern) · `serve_ref` (fingerprint of the originating serve — the **pairing reference**) | **The load-bearing event.** Episode-level: a memory fired for THIS need → did THIS need resolve — never session-level pass/fail. Welded to memory CID + session identity; expensive to fake (evidence-referenced). Resolution is a **tri-state** per WO-ATTRIB 2026-08-07 (§8.7 F6 reconciliation); `serve_ref` doubles as the pairing token (D-RECALL-PAIRING-TOKEN, §8.7). **Harvested, not reported** (§6). |
...
| **WITHHOLD** | Can a hub (or WeVibe) stop a consumer's evidence from entering the record? | Events are consumer-signed; submission runs through the hub relay, the **only** path the chain accepts (it rejects any other transaction signer) — a consumer cannot broadcast around a refusing hub today. Ruled 2026-08-07 (§8.7): a hub that will not relay is a **subscription failure** on the org-to-consumer pathway, not a breach of the withholding guarantee — the guarantees hold between the chain and the consuming infrastructure: once events are on chain, no party can keep them from the consumer, and standing recomputes from the same public stream. Gas rides the org feegrant (inherited canon, unchanged — the consumer needs no wallet). | **HOLDS — reframed** (the prior "strengthened — submission path no longer hub-mediated" cited an unbuilt direct-broadcast path the chain would reject) |
...
| mcp | **NEW** — outcome-event (E3) emission | The use-leg client leg: harvest signals → sign → submit via the hub relay (sole built path; the chain rejects any other transaction signer) |
