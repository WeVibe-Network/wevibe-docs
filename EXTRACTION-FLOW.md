...
4. **Episode segmentation pre-pass** **[CURRENT-TRUTH — built + live; canon D-FAILURE-EPISODES (`DECISIONS.md:905`)]** — deterministic code-only pre-pass (no LLM) runs before `planChunkSlices` (`:700-720`): failing signal → following edits → signal disappearance (resolved) or persistence (unresolved). Built: `segmentFailureEpisodes` (`wevibe-mcp/src/failure-episodes.ts:246`), grouped by `checkKey` (`test:<tool>` / `cmd:<verb>` / `tool:<tool>`); per-episode resolution `resolved | unresolved | coincidental`, episode `id` from the event index (`ep-<eventIndex>`). Resolved episodes yield symptom→diff→validation evidence spans; unresolved episodes yield dnd-only negative candidates ("tried X, symptom Y, unresolved") **— but only within a session that resolved ≥1 problem (see the progress invariant below)**. E2 evidence-bounding gates stay unchanged (this widens what extraction sees, never what it may claim); attribution uses the full attempt diff and discloses coincidental flips.

  **Reconciliation — shared segmentation + stable episode identity [DESIGN-INTENT; D-RECALL-KEY-SPLIT + D-RECALL-EPISODE-IS-A-FAILURE (`RECALL-PIVOT-SPEC.md:332-337`)]:** this deterministic failure-episode segmentation is now **SHARED between contribution and observation** (not contribution-only). **EPISODE IDENTITY IS THE STABLE `failureKey`** — derived from the predicate identity and the failing test identity; stable across attempts. `needSignature` is **VOLATILE**, recomputed per attempt, unchanged construction — it is the **QUERY MATERIAL only**, no longer the episode identity. **Superseded:** the old identity (`episodeRef = sha256("wevibe-episode-v1\norgId\nsessionId\nneedSignature")`) was volatile — a repeat produced a different `needSignature` and expired the prior episode (a repeat was unobservable). A change of need signature no longer expires an episode: an episode opens on the first red under a `failureKey`, accumulates attempts, and closes on green under the same key (worked) or on session end/idle (unobserved). **[CURRENT-TRUTH 2026-08-07]** the outcome resolution is **TRI-STATE `worked | didnt_work | unobserved`** (WO-ATTRIB), replacing the binary worked bit (`wevibe-opencode-plugin/plugins/outcome-episode.ts:19`).
