...
> **Forward note (Sprint 32 — `D-RECALL-GATE-ON-RISK`, `D-FAVORITE-PINNED-REFERENCE`).** To prevent popup fatigue — and the blind-accept habit it breeds, which turns the gate into theater and poisons Earned Trust (D-4.2) — the injection gate is moving to a third **`[Gated on risk]`** mode: auto-inject + attest the safe majority, and raise a blocking modal ONLY for guard-flagged / low-trust / low-confidence candidates, with the attest/deny quality signal batched into a review tray the consumer clears at a natural break. Negative-signal (DND) memories may be blind-accepted (still wevibe-guard-scanned — only the modal is skipped). A consumer "favorite" is a **pinned reference** (re-fetched and ranked high through the live path), never a local plaintext snapshot — preserving revocation, freshness, and serve attribution. The current `risk_appetite` field + `WEVIBE_ALLOW_UNREVIEWED` flag remain the live code until the D-11.5 matrix rename lands.
...
**Reputation is keyed by the passkey identity (not the wallet).** A contributor earns reputation/XP under their passkey pubkey from the moment they contribute — no wallet required. Connecting a wallet (for earnings/authority) does **not** by itself move reputation. Memory-contribution XP already keys by the passkey pubkey; serve XP must do the same. Earnings work the same way: they accrue on-chain to the passkey identity as a claim-later balance (the memory already records the contributor's passkey id), NOT to the wallet.

**Wallet upgrade = LINK, reputation move = a separate, explicit MIGRATION.** Carrying reputation onto a wallet is a deliberate **on-chain, dual-signed alias** (passkey pubkey → wallet address), gated by the contributor's own memory-contribution trail, recorded with an `is_migrated` flag on chain (NOT a hub-DB record). Until migration, reputation stays keyed to the passkey pubkey; after migration it resolves to the wallet via the alias. (DECISIONS.md `D-REPUTATION-KEYED-BY-PUBKEY`, `D-MIGRATION-ONCHAIN-ALIAS`.) Linking a wallet ≠ migrating: linking only puts a wallet on file (withdrawal authority + leader/bond); the dual-signed `is_migrated` migration is what unlocks withdrawal of the accrued earnings to the wallet and collapses the UI to wallet-only. A user who connects a wallet before ever using the passkey identity has nothing to migrate, so migration auto-completes at link time (they never see the dual-render state). The non-migrated state is served from the chain keyed by the passkey pubkey — there is no hub-DB identity/earnings store (the hub may cache for speed only).
...
**Secure-your-earnings nudge.** Because real economic value accrues to the passkey identity, a lost passkey with no backup loses unclaimed earnings. Once a non-trivial balance accrues, the earnings UI surfaces a one-time "secure your earnings" prompt (BIP39 break-glass / link a wallet) — gated on actually having something to lose, so the zero-friction path is otherwise untouched. (DECISIONS.md `D-MIGRATION-ONCHAIN-ALIAS`.)
...
| Item | Reference | Severity | Status |
|------|-----------|----------|--------|
| Chain fast-epoch primitive (configurable `WEVIBE_EPOCH_DURATION_SECONDS`) + empirical replay harness + sprint contract measurement (`chain.gap ≥ 75pp vs sim Steady-State, |Δ| ≤ 5pp`) | CO-034 | CRITICAL | **In flight** |

### Sprint Contract

The Sprint 32 empirical contract is:

- `chain.gap ≥ 75pp` against the sim Steady-State scenario (badRate=0.12, tpDeny=0.55, fpDeny=0.04, qPerEpoch=15, qSize=3, servePer=3, initMem=100, maxKw=7).
- `|chain.gap − sim.gap| ≤ 5pp` for the same scenario.

The sim baseline is the QS3b combined model (D-4.2 Earned Trust + D-9.4 probabilistic exploration) at 79.5pp per `wevibe-sim/ranking-fix.js`. CO-034 measures the chain against this baseline.

**Status until CO-034 lands:** Sprint dependency closure is complete (all ingress wiring lives and the chain accepts matched_keywords end-to-end). Sprint contract validation is in flight.
